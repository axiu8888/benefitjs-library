...
export namespace collector {
   ...
   * 采集器包头
   */
  export const HEAD = [0x55, 0xaa];

  /**
   * 采集器设备
   */
  export class Device implements IDevice, DataListener {
    /**
     * 拼包器
     */
    ...
    constructor(listener?: DataListener) {
  ...
  export class Parser {
    ...
    convertToUdp(bytes: number[], deviceId: any, time?: number, packetSn?: number) {
      ...
      // 设备ID
      if (deviceId) {
        deviceId = deviceId.length == 4 ? deviceId : binary.hexToBytes(deviceId);
        // 4 ~ 7
    ...
    parse(data: number[], deviceId: string) {
      ...
      if (hp.type == 0xf3) {
        ...
        for (let i = 0, j = 9 + 544; i < 25; i++, j += 5) {
          breath[i] = data[i + j];
          realtimeFlowVelocity[i] = binary.bytesToNumber([data[i + j + 1], data[i + j + 2]]);
          realTimeVolume[i] = binary.bytesToNumber([data[i + j + 3], data[i + j + 4]]);
        }
        hp.breath = breath;
      ...
      if (hp.realtime) {
        ...
        if (batteryType != BatteryType.collector_inner && batteryType != BatteryType.collector_outer) {
          this.putBatteryLevel(deviceId, batteryType, data[531]);
        } else {
          let power = Math.floor((((data[531] - 15) * 5 + 3200 - 3300) / (4050 - 3300)) * 100);
          this.putBatteryLevel(deviceId, batteryType, Math.max(Math.min(power, 100), 0));
        }
      }
      // 0：内部电池
      hp.deviceBattery = this.getBatteryLevel(deviceId, BatteryType.collector_inner);
      // 1：外部电池
      hp.deviceOuterBattery = this.getBatteryLevel(deviceId, BatteryType.collector_outer);
      // 2：体温计电池
      hp.temperatureBattery = this.getBatteryLevel(deviceId, BatteryType.thermometer);
      // 3：血氧计电池
      hp.spo2Battery = this.getBatteryLevel(deviceId, BatteryType.oximeter);
      // 4：血压计电池
      hp.elecMmhgBattery = this.getBatteryLevel(deviceId, BatteryType.sphygmomanometer);
      // 5：流速仪
  ...
  export const packet_lost_retry = new PacketType('lost_retry', 0x08, false, true, true, false, '丢包重传：指令');
  ...
  export const packet_feedback_lost_retry = new PacketType('feedback_lost_retry', 0x83, true, false, true, false, '丢包重传(响应)：请求集中重传数据包');
   ...
   * @returns 返回指令
   */
  export const retryCmd = (deviceId: number[] | string, sn: number, count: number) => {
    let cmd = wrapCmd(deviceId, 0x08, 15);
    cmd[9] = Math.min(10, Math.max(1, count));
  ...
  export class PacketJointer {
    ...
    verify(jp: JointPacket): boolean {
      return jp.pkg0 && jp.pkg1 && jp.pkg2 && jp.pkg0[8] != 0xf3;
    }
   ...
   * @return 返回心电波形
   */
  export const toECGWave_10bits = (wave: number[], ad: number) => {
