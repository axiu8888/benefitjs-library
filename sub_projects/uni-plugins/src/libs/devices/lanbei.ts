...
export namespace lanbei {
  ...
  export class Client extends bluetooth.BluetoothClient<Client> {
    ...
    private readonly _calls = new Map<CmdType, Set<LanBeiCall>>();
      ...
      return new Promise<LanBeiResponse>((resolve, reject) => {
        ...
        if (type && type != cmd_ack) {
          let call = <LanBeiCall>{
            ...
            reject: (err: any) => {
              try {
                ...
                this._calls.get(call.type)?.delete(call);
              }
            },
          ...
          try {
            ...
            setTimeout(() => {
              if (type) {
                this._calls.get(type)?.delete(call);
              }
  ...
  export const cmd_types = [cmd_getStatus, cmd_getVersion, cmd_getTime, cmd_startCollector, cmd_stopCollector, cmd_setTime, cmd_setUser, cmd_setSN, cmd_clear, cmd_ack];
  /**
   * 查找指令类型
   */
