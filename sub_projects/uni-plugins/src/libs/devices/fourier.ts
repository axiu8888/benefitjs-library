...
export namespace h1 {
  ...
  export class Client extends bluetooth.BluetoothClient<Client> {
     ...
     * 发送指令
     */
    send(type: number, ...payload: number[]) {
      return this.write(wrapCmd(type, ...payload));
    }

    /**
     * 1.	呼叫命令：主要用于上位机呼叫下位机，测试通讯线路等
     */
    sendHandshakeCmd() {
      return this.send(0x11);
    }

    /**
     * 2.	启停参数上报：用于开启或关闭下位机工作时实时参数上报，0x00:表示停止上报  0x01:开启上报
     */
    sendRealtimeUploadEnableCmd(on: boolean) {
      return this.send(0x12, on ? 0x01 : 0x00);
    }
     ...
     * 命令码：0x13
     * 通讯过程描述：
     * 上位机 →→主被动：0x4D 0x2D 0x3E 0x53 0x09 0x13
     * 0x01 0x02 0x01 0x03 0x05 0x06
     * 0x00 0x00 0x42
     * 上位机 ←← 主被动：0x4D 0x3C 0x2D 0x53 0x01 0x13 0x48
     * <p>
     * 说明：
     * 上位机传递到下位机的数据依次为：
     * <p>
     *
     * @param mode      模式	模式 被动1 主动2 智能被动3 智能主动4
     * @param turnOn    启/停  暂停 暂停1 非暂停2 停止3(进入暂停状态并清除累积的里程和时间)
     * @param direction   方向 方向 正向1 反向2
     * @param duration    设定工作时间min  工作时间-分钟 1-255
     * @param speed     设定速度   设定速度r/min 1-60
     * @param spasmLevel  设定痉挛等级  设定痉挛等级 1-12
     * @param obstructionOn 智能阻力是否开启  智能阻力是否开启 关闭1 开启2
     * @param obstruction   设定阻力  设定阻力等级 1-12
     * @return 是否发送
     * 注:如果仅设置某个参数时,其余不想设置的参数保持为0即可.(范围外的参数将不被接受)
     */
    sendConfCmd2(mode: number, turnOn: number, direction: number, duration: number, speed: number, spasmLevel: number, obstructionOn: number, obstruction: number) {
      return this.send(0x13, mode, turnOn, direction, duration, speed, spasmLevel, obstructionOn, obstruction);
    }

    /**
     * 4.	读取工作参数:读取下位机现有设置参数(下位机参数有变时,也会主动上报)
     * <p>
     * 命令码:0x14
     * 通讯过程描述：
     * 上位机 →→主被动：0x4D 0x2D 0x3E 0x53 0x01 0x14 0x4F
     * 上位机←← 主被动：0x4D 0x3C 0x2D 0x53 0x09 0x14
     * 0x01 0x02 0x01 0x03 0x05 0x06
     * 0x01 0x06 0x42
     * 参数顺序及含义同3
     */
    sendReadConfCmd() {
      return this.send(0x14);
    }
  ...
  export interface Listener extends uniapp.OnBleClientListener<Client> {
     ...
     * @param value 数值
     * @param opts 参数: FourierOptions
     */
    onOptions(client: Client, deviceId: string, value: number[] | Array<number> | Uint8Array, opts: Options): void;
  ...
  export const train_mode_passive = <H1TrainMode>{ type: 0x01, name: '被动', alias: 'passive' };
  export const train_mode_active = <H1TrainMode>{ type: 0x02, name: '主动', alias: 'active' };
  ...
  export const h1_train_modes = [train_mode_passive, train_mode_active, train_mode_intelligent_passive, train_mode_intelligent_active];
  ...
  export const resolvePacket = (data: number[]): Packet => {
    let mode = h1_train_modes.find((tm) => tm.type == (data[6] & 0xff));
    if (!mode) {
...
export namespace a4 {
  ...
  export class Client extends bluetooth.BluetoothClient<Client> {
        ...
        while (buf.size() > 4) {
          ...
          if (index >= 0) {
            ...
            if (buf.size() >= len) {
              ...
              client.resolve(deviceId, segment);
            }
          } else {
            buf.clear(); // 丢弃
          }
    ...
    send(state: number): Promise<uniapp.UniBtDeviceResponse> {
      ...
      return this.write(wrapCmd(0x40, data));
    }
  ...
  export interface Listener extends uniapp.OnBleClientListener<Client> {
     ...
     * @param packet 解析的数据包
     */
    onData(client: Client, deviceId: string, value: number[] | Uint8Array | ArrayBuffer, packet: Packet): void;
  }
   ...
   * @return 返回数据
   */
  function getPayload(data: number[] | Uint8Array | ArrayBuffer) {
    // 55 1200 00 00 2100 00000100000000000032 68
    // 55 包头 1字节
    // 1200 长度 2字节，小端
    // 00 代码，或校验信息 可不用
