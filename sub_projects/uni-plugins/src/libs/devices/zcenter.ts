...
export namespace zcenter {
   ...
   * @returns 返回解析后的中央台数据
   */
  export function parseZCenter(data: number[] | Uint8Array | ArrayBuffer): Packet {
  ...
  export function convertZCenterBytes(hp: c.HardwarePacket, mmhg?: c.BpPacket) {
    ...
    result[574] = hp.hrAlarm | (hp.hrAlarm << 2) | (hp.plusRateAlarm << 4);
  ...
  export class AlarmStatus {
     ...
     * 心电导联状态
     */
    isEcgFallOff(zp: Packet): boolean {
     ...
     * 血氧连接脱落报警
     */
    isSpo2FallOff(zp: Packet): boolean {
     ...
     * 呼吸率报警：0：无 1：低 2：高
     */
    getPlusRateAlarm(zp: Packet): number {
     ...
     * 外部电池异常报警
     */
    isDeviceOutBatteryAlarm(zp: Packet): boolean {
     ...
     * 血氧电池异常报警
     */
    isSpo2BatteryAlarm(zp: Packet): boolean {
