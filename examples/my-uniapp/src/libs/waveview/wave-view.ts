...
// 40毫秒执行一次
// 心电每秒200个值      每次绘制8个值
export function createEcg6(c: HTMLCanvasElement): WaveView {
  return new WaveView(c, {
    // 初始化
    onInit(view: WaveView) {
      let canvas = view.canvas;
      let step = 1.0;
      let baseLineHalf = (canvas.height / 6) / 2;
      // 添加ViewModel
      view.models.push(
        // 创建心电
        new ViewModel({
          width: canvas.width, // 宽度
          height: canvas.height / 6, // 高度
          drawCount: 8, // 绘制点数
          median: 512, // 中值 = (最大值 - 最小值) / 2
          step: step, // 步长
          baseLine: canvas.height * (1 / 6) - baseLineHalf, // 基线: 第1条线的2分之1
          maxCacheSize: 2, // 缓存数量
          scaleRatio: 0.5, // 缩放比
          padding: 16, // 空白填充
          startX: 0,
        ...
        new ViewModel({
          ...
          median: 512, // 中值 = (最大值 - 最小值) / 2
          step: step, // 步长
          baseLine: canvas.height * (2 / 6) - baseLineHalf, // 基线: 第2条线的2分之1
          maxCacheSize: 2, // 缓存数量
          scaleRatio: 0.5, // 缩放比
          padding: 16, // 空白填充
          startX: 0,
          startY: canvas.height * (1 / 6),
          strokeStyle: '#FF0000',
          clearDirty: true, // 擦除
        }),
        // 创建心电
        new ViewModel({
          ...
          median: 512, // 中值 = (最大值 - 最小值) / 2
          step: step, // 步长
          baseLine: canvas.height * (3 / 6) - baseLineHalf, // 基线: 第3条线的2分之1
          maxCacheSize: 2, // 缓存数量
          scaleRatio: 0.5, // 缩放比
          padding: 16, // 空白填充
          startX: 0,
          startY: canvas.height * (2 / 6),
          strokeStyle: '#FF0000',
          clearDirty: true, // 擦除
        }),
        // 创建心电
        new ViewModel({
          ...
          median: 512, // 中值 = (最大值 - 最小值) / 2
          step: step, // 步长
          baseLine: canvas.height * (4 / 6) - baseLineHalf, // 基线: 第4条线的2分之1
          maxCacheSize: 2, // 缓存数量
          scaleRatio: 0.5, // 缩放比
          padding: 16, // 空白填充
          startX: 0,
          startY: canvas.height * (3 / 6),
          strokeStyle: '#FF0000',
          clearDirty: true, // 擦除
        }),
        // 创建心电
        new ViewModel({
          ...
          median: 512, // 中值 = (最大值 - 最小值) / 2
          step: step, // 步长
          baseLine: canvas.height * (5 / 6) - baseLineHalf, // 基线: 第5条线的2分之1
          maxCacheSize: 2, // 缓存数量
          scaleRatio: 0.5, // 缩放比
          padding: 16, // 空白填充
          startX: 0,
          startY: canvas.height * (4 / 6),
          strokeStyle: '#FF0000',
          clearDirty: true, // 擦除
        }),
        // 创建心电
        new ViewModel({
          ...
          median: 512, // 中值 = (最大值 - 最小值) / 2
          step: step, // 步长
          baseLine: canvas.height * (6 / 6) - baseLineHalf, // 基线: 第6条线的2分之1
          maxCacheSize: 2, // 缓存数量
          scaleRatio: 0.5, // 缩放比
          padding: 16, // 空白填充
          startX: 0,
          startY: canvas.height * (5 / 6),
          strokeStyle: '#FF0000',
          clearDirty: true, // 擦除
        }),
      );
